...
export function SearchPage() {
  ...
  const handleSearch = async () => {
    ...
    if (searchParams.account) {
      const accountValidation = validationApi.validateAccountNumber(
    ...
    if (searchParams.actualDate) {
      const dateValidation = validationApi.validateDate(
    ...
    const hasParams = Object.values(searchParams).some(
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Связанные лица */}
      {selectedClient &&
        displayOptions.searchExtRelatedPersons &&
        relatedPersons && (
