...
export class LoggerService implements NestLoggerService {
  ...
  private initLogFiles(): void {
    ...
    this.logDir =
      this.config.get('LOG_DIR') || path.join(process.cwd(), 'logs');
  ...
  private logToConsole(entry: LogEntry): void {
    ...
    if (entry.level === 'error') {
      console.error(`${timestamp} ${level} ${context} ${entry.message}`);
      if (entry.stack) {
        console.error(entry.stack);
      }
    } else if (entry.level === 'warn') {
      console.warn(`${timestamp} ${level} ${context} ${entry.message}`);
    } else {
  ...
  private async handleCriticalError(entry: LogEntry): Promise<void> {
    ...
    if (
      entry.level === 'error' &&
      (entry.stack || messageStr.includes('FATAL'))
    ) {
  ...
  private createErrorSummary(errors: LogEntry[]): LogEntry {
    const uniqueContexts = [
      ...new Set(errors.map((e) => e.context).filter(Boolean)),
    ];
        ...
        attachments: [
          {
            ...
            fields: [
              ...
              { short: true, title: 'Время', value: timestamp },
              {
                short: false,
                title: 'Сообщение',
                value: entry.message.substring(0, 500),
              },
            ],
            ...(entry.stack && {
